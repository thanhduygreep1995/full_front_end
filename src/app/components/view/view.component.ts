...
export class ViewComponent {
  ...
  onClick() {
    const canActivate = this.loginAuth.canActivate(
    ...
    if (canActivate) {
      ...
      `,
        ...
        if (result.isConfirmed) {
          ...
          //       console.log('check customers', r.customer.id);
          //       if (
          //         r.rating != null &&
          ...
          //     }
          //     if (
          //       this.customerId == i.customers.id &&
          ...
          // });
          this.rate
            .getMyRating(this.id, this.customerId)
            .subscribe((rating) => {
              this.checkData = rating;
              if (this.checkData != null) {
                ...
                this.checkList = true;
              } else if (this.checkList == false && this.checkData == null) {
                const ratingForm = {
                  nameCustomer: this.customerResponse?.first_name+' '+ this.customerResponse?.last_name,
                  // i.customers.firstName + ' ' + i.customers.lastName,
                  rating: selectedValue,
                  createDate: new Date(),
                  updateDate: new Date(),
  ...
  generateQRCode() {
    const modifiedInfoProduct = this.modifyInfoProduct(
      this.infoProduct,
      this.Spec
    );
  ...
  modifyInfoProduct(infoProduct: any, spec: any): String {
    return (
      ...
      infoProduct.name +
      ', Giá đang bán: ' +
      infoProduct.discountPercentage +
      ', Mô tả sản phẩm: ' +
      infoProduct.description +
      ', Đang được giảm giá: ' +
      infoProduct.discountPrice +
      ', Loại sản phẩm: ' +
      infoProduct.categoryId.name +
      ', Hãng sản xuất: ' +
      infoProduct.brandId.name +
      ', Sản phẩm được xuất xứ: ' +
      infoProduct.originId.country +
      ', Số lượng còn lại: ' +
      ...
      ', Bộ nhớ thiết bị: ' +
      spec.storage +
      ', Kích thước chất lượng màn hình: ' +
      spec.display +
      ', Hệ điều hành & nhà sản xuất: ' +
  ...
  loadFeedback() {
    this.fB.getFeedBackProduct(this.id).subscribe((data) => {
      ...
      for (let i of this.feedBacks) {
        ...
        i.updateDate = this.datePipe.transform(
          dateObject,
          'dd/MM/yyyy - hh:mm'
        );
